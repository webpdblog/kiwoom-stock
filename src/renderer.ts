...
document.addEventListener('DOMContentLoaded', () => {
  ...
  apiSelector.addEventListener('change', async (event) => {
    ...
    if (actionId === 'au10002') {
    ...
    } else if (actionId === 'ka10020') {
      if (!mainContent) return;

      mainContent.innerHTML = `
        <h1>호가잔량상위요청 (ka10020)</h1>
        <div class="horizontal-form">
          <div class="form-row">
            <div class="form-field">
              <label for="market-type-input-ka10020">시장구분:</label>
              <select id="market-type-input-ka10020" name="market-type">
                <option value="001">코스피</option>
                <option value="101">코스닥</option>
              </select>
            </div>
            <div class="form-field">
              <label for="sort-type-input-ka10020">정렬구분:</label>
              <select id="sort-type-input-ka10020" name="sort-type">
                <option value="1">순매수잔량순</option>
                <option value="2">순매도잔량순</option>
                <option value="3">매수비율순</option>
                <option value="4">매도비율순</option>
              </select>
            </div>
            <div class="form-field">
              <label for="trade-qty-type-input-ka10020">거래량구분:</label>
              <select id="trade-qty-type-input-ka10020" name="trade-qty-type">
                <option value="0000">장시작전(0주이상)</option>
                <option value="0010">만주이상</option>
                <option value="0050">5만주이상</option>
                <option value="0100">10만주이상</option>
              </select>
            </div>
            <div class="form-field">
              <label for="stock-condition-input-ka10020">종목조건:</label>
              <select id="stock-condition-input-ka10020" name="stock-condition">
                <option value="0">전체조회</option>
                <option value="1">관리종목제외</option>
                <option value="5">증100제외</option>
                <option value="6">증100만보기</option>
            ...
            <div class="form-field">
              <label for="credit-condition-input-ka10020">신용조건:</label>
              <select id="credit-condition-input-ka10020" name="credit-condition">
                <option value="0">전체조회</option>
                <option value="1">신용융자A군</option>
                <option value="2">신용융자B군</option>
                <option value="3">신용융자C군</option>
            ...
            <div class="form-field">
              <label for="exchange-type-input-ka10020">거래소구분:</label>
              <select id="exchange-type-input-ka10020" name="exchange-type">
                <option value="1">KRX</option>
                <option value="2">NXT</option>
                <option value="3">통합</option>
              </select>
            ...
            <div class="form-field">
              <button id="bid-ask-volume-upper-submit-btn">조회</button>
            </div>
          </div>
        </div>
        <div id="bid-ask-volume-upper-result"></div>
      `;

      const marketTypeInput = document.getElementById('market-type-input-ka10020') as HTMLSelectElement;
      const sortTypeInput = document.getElementById('sort-type-input-ka10020') as HTMLSelectElement;
      const tradeQtyTypeInput = document.getElementById('trade-qty-type-input-ka10020') as HTMLSelectElement;
      const stockConditionInput = document.getElementById('stock-condition-input-ka10020') as HTMLSelectElement;
      const creditConditionInput = document.getElementById('credit-condition-input-ka10020') as HTMLSelectElement;
      const exchangeTypeInput = document.getElementById('exchange-type-input-ka10020') as HTMLSelectElement;
      const submitBtn = document.getElementById('bid-ask-volume-upper-submit-btn') as HTMLButtonElement;
      const resultDiv = document.getElementById('bid-ask-volume-upper-result') as HTMLDivElement;

      const fetchAndDisplayBidAskVolumeUpperData = async () => {
        resultDiv.innerHTML = 'Fetching bid/ask volume upper data...';
        
        try {
          const result = await window.electronAPI.invoke('get-bid-ask-volume-upper-data', {
            marketType: marketTypeInput.value,
            sortType: sortTypeInput.value,
            tradeQtyType: tradeQtyTypeInput.value,
            stockCondition: stockConditionInput.value,
          ...
          if (result.success) {
            const bidAskVolumeUpperData = result.bidAskVolumeUpperData;
            
            if (!bidAskVolumeUpperData || bidAskVolumeUpperData.length === 0) {
              resultDiv.innerHTML = '<p>조회된 데이터가 없습니다.</p>';
              return;
            }
            ...
            let tableHTML = '<table class="bid-ask-volume-upper-table"><thead><tr>';
            const headers = [
              '종목코드', '종목명', '현재가', '전일대비', '거래량', 
              '총매도잔량', '총매수잔량', '순매수잔량', '매수비율'
            ];
            headers.forEach(h => tableHTML += `<th>${h}</th>`);
            tableHTML += '</tr></thead><tbody>';

            bidAskVolumeUpperData.forEach((item: any) => {
              const currentPrice = formatNumber(item.cur_prc || '0');
              const previousDiff = formatNumber(item.pred_pre || '0');
              const volume = formatNumber(item.trde_qty || '0');
              const totalSellReq = formatNumber(item.tot_sel_req || '0');
              const totalBuyReq = formatNumber(item.tot_buy_req || '0');
              const netPurchaseReq = formatNumber(item.netprps_req || '0');
              const buyRate = item.buy_rt || '0';
              const signSymbol = getSignSymbol(item.pred_pre_sig);
              ...
              tableHTML += `<td>${signSymbol}${previousDiff}</td>`;
              tableHTML += `<td>${volume}</td>`;
              tableHTML += `<td>${totalSellReq}</td>`;
              tableHTML += `<td>${totalBuyReq}</td>`;
              tableHTML += `<td>${netPurchaseReq}</td>`;
              tableHTML += `<td>${buyRate}%</td>`;
              tableHTML += '</tr>';
            });
    ...
    } else if (actionId === 'ka10022') {
      if (!mainContent) return;

      mainContent.innerHTML = `
        <h1>잔량율급증요청 (ka10022)</h1>
        <div class="horizontal-form">
          <div class="form-row">
            <div class="form-field">
              <label for="market-type-input-ka10022">시장구분:</label>
              <select id="market-type-input-ka10022" name="market-type">
                <option value="001">코스피</option>
                <option value="101">코스닥</option>
              </select>
            </div>
            <div class="form-field">
              <label for="ratio-type-input-ka10022">비율구분:</label>
              <select id="ratio-type-input-ka10022" name="ratio-type">
                <option value="1">매수/매도비율</option>
                <option value="2">매도/매수비율</option>
              </select>
            </div>
            <div class="form-field">
              <label for="time-type-input-ka10022">시간구분(분):</label>
              <input type="number" id="time-type-input-ka10022" name="time-type" value="30" min="1" max="99" />
            </div>
            <div class="form-field">
              <label for="trade-qty-type-input-ka10022">거래량구분:</label>
              <select id="trade-qty-type-input-ka10022" name="trade-qty-type">
                <option value="5">5천주이상</option>
                <option value="10">만주이상</option>
                <option value="50">5만주이상</option>
                <option value="100">10만주이상</option>
            ...
            <div class="form-field">
              <label for="stock-condition-input-ka10022">종목조건:</label>
              <select id="stock-condition-input-ka10022" name="stock-condition">
                <option value="0">전체조회</option>
                <option value="1">관리종목제외</option>
                <option value="5">증100제외</option>
                <option value="6">증100만보기</option>
            ...
            <div class="form-field">
              <label for="exchange-type-input-ka10022">거래소구분:</label>
              <select id="exchange-type-input-ka10022" name="exchange-type">
                <option value="1">KRX</option>
                <option value="2">NXT</option>
                <option value="3">통합</option>
              </select>
            ...
            <div class="form-field">
              <button id="balance-rate-surge-submit-btn">조회</button>
            </div>
          </div>
        </div>
        <div id="balance-rate-surge-result"></div>
      `;

      const marketTypeInput = document.getElementById('market-type-input-ka10022') as HTMLSelectElement;
      const ratioTypeInput = document.getElementById('ratio-type-input-ka10022') as HTMLSelectElement;
      const timeTypeInput = document.getElementById('time-type-input-ka10022') as HTMLInputElement;
      const tradeQtyTypeInput = document.getElementById('trade-qty-type-input-ka10022') as HTMLSelectElement;
      const stockConditionInput = document.getElementById('stock-condition-input-ka10022') as HTMLSelectElement;
      const exchangeTypeInput = document.getElementById('exchange-type-input-ka10022') as HTMLSelectElement;
      const submitBtn = document.getElementById('balance-rate-surge-submit-btn') as HTMLButtonElement;
      const resultDiv = document.getElementById('balance-rate-surge-result') as HTMLDivElement;

      const fetchAndDisplayBalanceRateSurgeData = async () => {
        resultDiv.innerHTML = 'Fetching balance rate surge data...';
        
        try {
          const result = await window.electronAPI.invoke('get-balance-rate-surge-data', {
            marketType: marketTypeInput.value,
            ratioType: ratioTypeInput.value,
            timeType: timeTypeInput.value.padStart(2, '0'),
            tradeQtyType: tradeQtyTypeInput.value,
            stockCondition: stockConditionInput.value,
            exchangeType: exchangeTypeInput.value,
          ...
          if (result.success) {
            const balanceRateSurgeData = result.balanceRateSurgeData;
            
            if (!balanceRateSurgeData || balanceRateSurgeData.length === 0) {
              resultDiv.innerHTML = '<p>조회된 데이터가 없습니다.</p>';
              return;
            }
            ...
            let tableHTML = '<table class="balance-rate-surge-table"><thead><tr>';
            const headers = [
              '종목코드', '종목명', '현재가', '전일대비', '기준률', 
              '현재비율', '급증률', '총매도잔량', '총매수잔량'
            ];
            headers.forEach(h => tableHTML += `<th>${h}</th>`);
            tableHTML += '</tr></thead><tbody>';

            balanceRateSurgeData.forEach((item: any) => {
              const currentPrice = formatNumber(item.cur_prc || '0');
              const previousDiff = formatNumber(item.pred_pre || '0');
              const baseRate = formatNumber(item.int || '0');
              const currentRate = formatNumber(item.now_rt || '0');
              const surgeRate = item.sdnin_rt || '0';
              const totalSellReq = formatNumber(item.tot_sel_req || '0');
              const totalBuyReq = formatNumber(item.tot_buy_req || '0');
              const signSymbol = getSignSymbol(item.pred_pre_sig);
      ...
      submitBtn.addEventListener('click', fetchAndDisplayBalanceRateSurgeData);
    } else if (actionId === 'ka10023') {
      if (!mainContent) return;

      mainContent.innerHTML = `
        <h1>거래량급증요청 (ka10023)</h1>
        <div class="horizontal-form">
          <div class="form-row">
            <div class="form-field">
              <label for="market-type-input-ka10023">시장구분:</label>
              <select id="market-type-input-ka10023" name="market-type">
                <option value="000">전체</option>
                <option value="001">코스피</option>
                <option value="101">코스닥</option>
              </select>
            ...
            <div class="form-field">
              <label for="sort-type-input-ka10023">정렬구분:</label>
              <select id="sort-type-input-ka10023" name="sort-type">
                <option value="1">급증량</option>
                <option value="2">급증률</option>
                <option value="3">급감량</option>
                <option value="4">급감률</option>
              </select>
            </div>
            <div class="form-field">
              <label for="time-type-input-ka10023">시간구분:</label>
              <select id="time-type-input-ka10023" name="time-type">
                <option value="1">분</option>
                <option value="2">전일</option>
              </select>
            </div>
            <div class="form-field">
              <label for="trade-qty-type-input-ka10023">거래량구분:</label>
              <select id="trade-qty-type-input-ka10023" name="trade-qty-type">
                <option value="5">5천주이상</option>
                <option value="10">만주이상</option>
                <option value="50">5만주이상</option>
                <option value="100">10만주이상</option>
            ...
            <div class="form-field">
              <label for="time-minute-input-ka10023">시간(분):</label>
              <input type="number" id="time-minute-input-ka10023" name="time-minute" min="1" max="99" placeholder="분 입력" />
            </div>
            <div class="form-field">
              <label for="stock-condition-input-ka10023">종목조건:</label>
              <select id="stock-condition-input-ka10023" name="stock-condition">
                <option value="0">전체조회</option>
                <option value="1">관리종목제외</option>
                <option value="3">우선주제외</option>
                <option value="11">정리매매종목제외</option>
                <option value="4">관리종목,우선주제외</option>
                <option value="5">증100제외</option>
                <option value="6">증100만보기</option>
                <option value="13">증60만보기</option>
                <option value="12">증50만보기</option>
                <option value="7">증40만보기</option>
                <option value="8">증30만보기</option>
                <option value="9">증20만보기</option>
                <option value="17">ETN제외</option>
                <option value="14">ETF제외</option>
                <option value="18">ETF+ETN제외</option>
                <option value="15">스팩제외</option>
                <option value="20">ETF+ETN+스팩제외</option>
              </select>
            </div>
            <div class="form-field">
              <label for="price-type-input-ka10023">가격구분:</label>
              <select id="price-type-input-ka10023" name="price-type">
                <option value="0">전체조회</option>
                <option value="2">5만원이상</option>
                <option value="5">1만원이상</option>
                <option value="6">5천원이상</option>
                <option value="8">1천원이상</option>
                <option value="9">10만원이상</option>
              </select>
            </div>
            <div class="form-field">
              <label for="exchange-type-input-ka10023">거래소구분:</label>
              <select id="exchange-type-input-ka10023" name="exchange-type">
                <option value="1">KRX</option>
                <option value="2">NXT</option>
                <option value="3">통합</option>
              </select>
            ...
            <div class="form-field">
              <button id="trading-volume-surge-submit-btn">조회</button>
            </div>
          </div>
        </div>
        <div id="trading-volume-surge-result"></div>
      `;

      const marketTypeInput = document.getElementById('market-type-input-ka10023') as HTMLSelectElement;
      const sortTypeInput = document.getElementById('sort-type-input-ka10023') as HTMLSelectElement;
      const timeTypeInput = document.getElementById('time-type-input-ka10023') as HTMLSelectElement;
      const tradeQtyTypeInput = document.getElementById('trade-qty-type-input-ka10023') as HTMLSelectElement;
      const timeMinuteInput = document.getElementById('time-minute-input-ka10023') as HTMLInputElement;
      const stockConditionInput = document.getElementById('stock-condition-input-ka10023') as HTMLSelectElement;
      const priceTypeInput = document.getElementById('price-type-input-ka10023') as HTMLSelectElement;
      const exchangeTypeInput = document.getElementById('exchange-type-input-ka10023') as HTMLSelectElement;
      const submitBtn = document.getElementById('trading-volume-surge-submit-btn') as HTMLButtonElement;
      const resultDiv = document.getElementById('trading-volume-surge-result') as HTMLDivElement;

      const fetchAndDisplayTradingVolumeSurgeData = async () => {
        resultDiv.innerHTML = 'Fetching trading volume surge data...';
        
        try {
          const result = await window.electronAPI.invoke('get-trading-volume-surge-data', {
            marketType: marketTypeInput.value,
            sortType: sortTypeInput.value,
            timeType: timeTypeInput.value,
            tradeQtyType: tradeQtyTypeInput.value,
            timeMinute: timeMinuteInput.value,
            stockCondition: stockConditionInput.value,
          ...
          if (result.success) {
            const tradingVolumeSurgeData = result.tradingVolumeSurgeData;
            
            if (!tradingVolumeSurgeData || tradingVolumeSurgeData.length === 0) {
              resultDiv.innerHTML = '<p>조회된 데이터가 없습니다.</p>';
              return;
            }
            ...
            let tableHTML = '<table class="trading-volume-surge-table"><thead><tr>';
            const headers = [
            ...
            tradingVolumeSurgeData.forEach((item: any) => {
              const currentPrice = formatNumber(item.cur_prc || '0');
              const previousDiff = formatNumber(item.pred_pre || '0');
              ...
              const surgeQty = item.sdnin_qty || '0';
              const surgeRate = item.sdnin_rt || '0';
              const signSymbol = getSignSymbol(item.pred_pre_sig);
